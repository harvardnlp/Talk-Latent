...
\begin{frame}\thetitle{Posterior Inference}
    ...
    Why?
    \begin{itemize}
        \item $z$ will often represent interesting information about our data.
...
\begin{frame}
\thetitle{Six Challenges for NMT (Koehn and Knowles 2017)}
\begin{enumerate}
    \item Out of domain generalization
    \air
    ...
    \item Long sentences
    \air
    \item \textbf{The alignments learned by soft attention may not
        be interpreted as word alignments}
    \air
    \item Beam search degradation for large beams
\end{enumerate}
\end{frame}
...
\begin{frame}\thetitle{Attention versus Alignment}
\begin{itemize}
     \item Attention is motivated as an ``alignment'' module:
        \begin{itemize}
            \item makes the model's behavior interpretable
            \item can be used for other prediction tasks
        \end{itemize}
    \air
    \item Soft Attention is a \textit{deterministic}  from a neural network,
    \air

    \item We contrast this with alignment which acts as a random variable.
    \end{itemize}
\end{frame}
...
{\small[Bahdanau et al 2014]}
    \begin{eqnarray*}
        \log \E_z [f(x, z;\theta)_{\hat{y}}] \approx \log f(x,\E_z[z];\theta)
    \end{eqnarray*}
...
\item {\small[Xu et al 2015]}: Directly apply Jensen's inequality and optimize
    ...
    \begin{eqnarray*}
        \log \E_z [f(x, z;\theta)_{\hat{y}}] \ge \E_z \log [f(x, z;\theta)_{\hat{y}}]
    \end{eqnarray*}
...
%   \thetitle{Conclusion}
%   \begin{itemize}
%   \item Latent alignment methods competitive or better than soft attention.
%     \item   Code: \url{https://github.com/harvardnlp/var-attn}
%   \item Future Work
%     \begin{itemize}
%     \item Utilize additional knowledge to help alignment.
%     \item Incorporate model constraints.
%     \item Variational alignment for other problems.
%     \end{itemize}
%   \end{itemize}
% \end{frame}
